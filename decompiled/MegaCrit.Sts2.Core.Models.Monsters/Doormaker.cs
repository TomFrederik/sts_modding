...
public sealed class Doormaker : MonsterModel
{
	...
	{
		await ((GodotObject)tween).ToSignal((GodotObject)(object)tween, SignalName.Finished);
		NCombatRoom.Instance?.RemoveCreatureNode(creatureNode);
		((Node)(object)creatureNode).QueueFreeSafely();
	}
	...
	private async Task AnimOut()
	{
		...
		if (nCreature != null)
		{
			...
			val.TweenProperty((GodotObject)(object)nCreature.Visuals.Body, NodePath.op_Implicit("scale"), Variant.op_Implicit(scale * 0.5f), 1.2000000476837158).SetEase((EaseType)1).SetTrans((TransitionType)1);
			val.Parallel().TweenProperty((GodotObject)(object)nCreature.Visuals.Body, NodePath.op_Implicit("modulate"), Variant.op_Implicit(Colors.Black), 0.25).From(Variant.op_Implicit(Colors.White));
			val.Parallel().TweenProperty((GodotObject)(object)nCreature.Visuals.Body, NodePath.op_Implicit("modulate"), Variant.op_Implicit(Colors.Transparent), 0.25).From(Variant.op_Implicit(Colors.Black))
				.SetDelay(0.25);
			TaskHelper.RunSafely(RemoveCreatureWhenDone(nCreature, val));
	...
	public async Task AnimIn()
	{
		...
		if (nCreature != null)
		{
			...
			Tween val = ((Node)nCreature).CreateTween();
			val.TweenProperty((GodotObject)(object)nCreature.Visuals.Body, NodePath.op_Implicit("scale"), Variant.op_Implicit(scale), 1.2000000476837158).From(Variant.op_Implicit(scale * 0.5f)).SetEase((EaseType)1)
				.SetTrans((TransitionType)1);
			val.Parallel().TweenProperty((GodotObject)(object)nCreature.Visuals.Body, NodePath.op_Implicit("modulate"), Variant.op_Implicit(Colors.Black), 0.25).From(Variant.op_Implicit(Colors.Transparent));
			val.Parallel().TweenProperty((GodotObject)(object)nCreature.Visuals.Body, NodePath.op_Implicit("modulate"), Variant.op_Implicit(Colors.White), 0.25).From(Variant.op_Implicit(Colors.Black))
				.SetDelay(0.25);
			TaskHelper.RunSafely(PlayLineAfterAnimIn(val));
			await Cmd.CustomScaledWait(0.2f, 0.6f);
		}
	}
