...
public sealed class BrainLeech : EventModel
{
	...
	private async Task Rip()
	{
		...
		{
			CardCreationOptions options = CardCreationOptions.ForNonCombatWithDefaultOdds(new _003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(ModelDb.CardPool<ColorlessCardPool>()));
	...
	private async Task ShareKnowledge()
	{
		...
		List<CardCreationResult> cards = CardFactory.CreateForReward(owner, base.DynamicVars["FromCardChoiceCount"].IntValue, CardCreationOptions.ForNonCombatWithDefaultOdds(new _003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(owner.Character.CardPool))).ToList();
