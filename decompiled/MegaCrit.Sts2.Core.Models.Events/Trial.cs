...
public sealed class Trial : EventModel
{
	...
	protected override IEnumerable<DynamicVar> CanonicalVars => new _003C_003Ez__ReadOnlySingleElementList<DynamicVar>(new DynamicVar("EntrantNumber", -1m));
	...
	private Task Accept()
	{
		...
		case 2:
			...
			eventOptions = new EventOption[2]
			{
				...
				new EventOption(this, NondescriptInnocent, "TRIAL.pages.NONDESCRIPT.options.INNOCENT", HoverTipFactory.FromCardWithCardHoverTips<Doubt>().Concat(new _003C_003Ez__ReadOnlySingleElementList<IHoverTip>(HoverTipFactory.Static(StaticHoverTip.Transform))))
			};
			break;
		default:
	...
	private async Task NondescriptGuilty()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			list.Add(new CardReward(CardCreationOptions.ForNonCombatWithDefaultOdds(new _003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(base.Owner.Character.CardPool)), 3, base.Owner));
		}
