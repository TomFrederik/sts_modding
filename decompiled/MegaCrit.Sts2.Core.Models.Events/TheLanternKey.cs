...
public sealed class TheLanternKey : EventModel
{
	...
	protected override IEnumerable<DynamicVar> CanonicalVars => new _003C_003Ez__ReadOnlySingleElementList<DynamicVar>(new GoldVar(100));
	...
	{
		SetEventState(L10NLookup("THE_LANTERN_KEY.pages.KEEP_THE_KEY.description"), new _003C_003Ez__ReadOnlySingleElementList<EventOption>(new EventOption(this, Fight, "THE_LANTERN_KEY.pages.KEEP_THE_KEY.options.FIGHT")));
		return Task.CompletedTask;
	}

	private Task Fight()
	{
		EnterCombatWithoutExitingEvent<MysteriousKnightEventEncounter>(new _003C_003Ez__ReadOnlySingleElementList<Reward>(new SpecialCardReward(base.Owner.RunState.CreateCard<LanternKey>(base.Owner), base.Owner)), shouldResumeAfterCombat: false);
		return Task.CompletedTask;
	}
}
