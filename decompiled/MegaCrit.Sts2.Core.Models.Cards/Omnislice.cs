...
public sealed class Omnislice : CardModel
{
	...
	protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
	{
		...
		{
			List<Creature> list2 = (from e in base.CombatState.GetTeammatesOf(damageResult.Receiver).Except(new _003C_003Ez__ReadOnlySingleElementList<Creature>(cardPlay.Target))
				where e.IsHittable
				select e).ToList();
			if (list2.Count != 0)
			{
