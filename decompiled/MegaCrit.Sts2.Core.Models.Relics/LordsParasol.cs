	...
	private async Task PurchaseEverything(MerchantInventory inventory)
	{
		...
		try
		{
			if (TestMode.IsOff)
			{
				...
				NHotkeyManager.Instance.AddBlockingScreen((Node)(object)NMerchantRoom.Instance.Inventory);
				await Cmd.Wait(1f);
			}
		...
		finally
		{
			if (uiBlocked)
			{
				NHotkeyManager.Instance.RemoveBlockingScreen((Node)(object)NMerchantRoom.Instance.Inventory);
