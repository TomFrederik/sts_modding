...
public sealed class ScrollBoxes : RelicModel
{
	...
	public static List<IReadOnlyList<CardModel>> GenerateRandomBundles(Player player)
	{
		...
		CardCreationOptions options = CardCreationOptions.ForNonCombatWithUniformOdds(new _003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(player.Character.CardPool), (CardModel c) => c.Rarity == CardRarity.Common).WithFlags(CardCreationFlags.NoRarityModification);
		options = Hook.ModifyCardRewardCreationOptions(player.RunState, player, options);
		CardCreationOptions options2 = CardCreationOptions.ForNonCombatWithUniformOdds(new _003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(player.Character.CardPool), (CardModel c) => c.Rarity == CardRarity.Uncommon).WithFlags(CardCreationFlags.NoRarityModification);
