	...
	public ModelId? AncientCard
	{
		...
		set
		{
			...
			if (_ancientCard != null)
			{
				...
				_extraHoverTips = cardModel.HoverTips.Concat(new _003C_003Ez__ReadOnlySingleElementList<IHoverTip>(HoverTipFactory.FromCard(cardModel, upgrade: true)));
				((StringVar)base.DynamicVars["AncientCard"]).StringValue = cardModel.Title;
			}
		}
	}

	protected override IEnumerable<DynamicVar> CanonicalVars => new _003C_003Ez__ReadOnlySingleElementList<DynamicVar>(new StringVar("AncientCard"));
