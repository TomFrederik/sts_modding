...
public class Draft : ModifierModel
{
	...
	{
		CardCreationOptions creationOptions = new CardCreationOptions(new _003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(player.Character.CardPool), CardCreationSource.Other, CardRarityOddsType.RegularEncounter).WithFlags(CardCreationFlags.NoUpgradeRoll);
		for (int i = 0; i < 10; i++)
		{
