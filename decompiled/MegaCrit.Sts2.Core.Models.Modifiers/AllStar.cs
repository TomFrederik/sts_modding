...
public class AllStar : ModifierModel
{
	...
	private static async Task ObtainCards(Player player, Rng rng)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			CardCreationOptions options = CardCreationOptions.ForNonCombatWithUniformOdds(new _003C_003Ez__ReadOnlySingleElementList<CardPoolModel>(ModelDb.CardPool<ColorlessCardPool>())).WithFlags(CardCreationFlags.NoRarityModification | CardCreationFlags.NoCardPoolModifications);
