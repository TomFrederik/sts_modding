...
public static class CardCmd
{
	public static async Task AutoPlay(PlayerChoiceContext choiceContext, CardModel card, Creature? target, AutoPlayType type = AutoPlayType.Default, bool skipXCapture = false, bool skipCardPileVisuals = false)
	{
		...
		if (!Hook.ShouldPlay(combatState, card, out AbstractModel preventer, type))
		{
			...
			{
				((Node)(object)NCombatRoom.Instance?.CombatVfxContainer).AddChildSafely((Node?)(object)NThoughtBubbleVfx.Create(playerDialogueLine.GetFormattedText(), card.Owner.Creature, 1.0));
			}
			return;
		}
	...
	{
		await Discard(choiceContext, new _003C_003Ez__ReadOnlySingleElementList<CardModel>(card));
	}
	...
	{
		Upgrade(new _003C_003Ez__ReadOnlySingleElementList<CardModel>(card), style);
	}
