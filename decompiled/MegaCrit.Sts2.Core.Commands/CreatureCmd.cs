...
public static class CreatureCmd
{
	...
	{
		return await Damage(choiceContext, new _003C_003Ez__ReadOnlySingleElementList<Creature>(target), amount, props, dealer, null);
	}
	...
	{
		return await Damage(choiceContext, new _003C_003Ez__ReadOnlySingleElementList<Creature>(target), damageVar.BaseValue, damageVar.Props, dealer, cardSource);
	}
	...
	{
		return await Damage(choiceContext, new _003C_003Ez__ReadOnlySingleElementList<Creature>(target), amount, props, dealer, cardSource);
	}
	...
	public static async Task<IEnumerable<DamageResult>> Damage(PlayerChoiceContext choiceContext, IEnumerable<Creature> targets, decimal amount, ValueProp props, Creature? dealer, CardModel? cardSource)
	{
		...
		foreach (DamageResult unblockedDamageResult in results)
		{
			...
			if (unblockedDamageResult.WasFullyBlocked)
			{
				...
				val?.AddChildSafely((Node?)(object)NBlockSparkVfx.Create(unblockedDamageResult.Receiver));
				val?.AddChildSafely((Node?)(object)NDamageBlockedVfx.Create(unblockedDamageResult.Receiver));
				NGame.Instance?.ScreenShake(ShakeStrength.Weak, ShakeDuration.Short);
			}
		}
	...
	{
		await Kill(new _003C_003Ez__ReadOnlySingleElementList<Creature>(creature), force);
	}
	...
	private static async Task KillWithoutCheckingWinCondition(Creature creature, bool force, int recursion = 0)
	{
		...
		CombatState combatState = creature.CombatState;
		IRunState runState = IRunState.GetFrom(new _003C_003Ez__ReadOnlySingleElementList<Creature>(creature));
	...
	public static async Task Heal(Creature creature, decimal amount, bool playAnim = true)
	{
		...
		if (playAnim)
		{
			if (CombatManager.Instance.IsInProgress || creature.Player?.RunState.CurrentRoom is CombatRoom)
			{
				...
				((Node)(object)NCombatRoom.Instance?.CombatVfxContainer)?.AddChildSafely((Node?)(object)NHealNumVfx.Create(creature, amount));
				if (isDead)
				{
			...
			else if (LocalContext.IsMe(creature))
			{
				...
				{
					PlayerFullscreenHealVfx.Play(creature.Player, amount, (Control?)(object)NMerchantRoom.Instance);
				}
			}
