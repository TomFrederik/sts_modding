...
public static class CardPileCmd
{
	...
	{
		await RemoveFromDeck(new _003C_003Ez__ReadOnlySingleElementList<CardModel>(card), showPreview);
	}
	...
	{
		await RemoveFromCombat(new _003C_003Ez__ReadOnlySingleElementList<CardModel>(card), isBeingPlayed, skipVisuals);
	}
	...
	{
		return (await AddGeneratedCardsToCombat(new _003C_003Ez__ReadOnlySingleElementList<CardModel>(card), newPileType, addedByPlayer, position))[0];
	}
	...
	{
		return (await Add(new _003C_003Ez__ReadOnlySingleElementList<CardModel>(card), newPile, position, source, skipVisuals))[0];
	}
	...
	public static async Task<IReadOnlyList<CardPileAddResult>> Add(IEnumerable<CardModel> cards, CardPile newPile, CardPilePosition position = CardPilePosition.Bottom, AbstractModel? source = null, bool skipVisuals = false)
	{
		...
		if (cardNodes.Count != 0)
		{
			...
			foreach (NCard cardNode2 in cardNodes)
			{
				...
				if (flag8)
				{
					val.Parallel().TweenProperty((GodotObject)(object)cardNode2, NodePath.op_Implicit("position"), Variant.op_Implicit(((Control)cardNode2).Position + Vector2.Down * 25f), (double)((SaveManager.Instance.PrefsSave.FastMode == FastModeType.Fast) ? 0.2f : 0.3f));
					val.Parallel().TweenProperty((GodotObject)(object)cardNode2, NodePath.op_Implicit("modulate"), Variant.op_Implicit(StsColors.exhaustGray), (double)((SaveManager.Instance.PrefsSave.FastMode == FastModeType.Fast) ? 0.2f : 0.3f));
					val.Chain().TweenCallback(Callable.From((Action)cardNode2.QueueFreeSafely));
					continue;
				}
				...
				case PileType.Exhaust:
					...
					{
						AppendPileLerpTween(val, cardNode2, PileType.Play, oldPile2);
					...
					val.Chain().TweenCallback(Callable.From((Action)delegate
					{
						((Node)(object)NCombatRoom.Instance.Ui).AddChildSafely((Node?)(object)NExhaustVfx.Create(cardNode2));
					}));
					val.Parallel().TweenProperty((GodotObject)(object)cardNode2, NodePath.op_Implicit("modulate"), Variant.op_Implicit(StsColors.exhaustGray), (double)((SaveManager.Instance.PrefsSave.FastMode == FastModeType.Fast) ? 0.2f : 0.3f));
					val.Chain().TweenCallback(Callable.From((Action)cardNode2.QueueFreeSafely));
					break;
				case PileType.Hand:
					AppendPileLerpTween(val, cardNode2, card3.Pile.Type, oldPile2);
					...
					break;
				case PileType.Play:
					AppendPlayPileLerpTween(val, cardNode2, oldPile2);
					break;
				default:
					val.TweenCallback(Callable.From((Action)delegate
					{
						...
						Node val2 = (Node)((card3.Pile.Type != PileType.Deck) ? ((object)NCombatRoom.Instance.CombatVfxContainer) : ((object)NRun.Instance.GlobalUi.TopBar.TrailContainer));
						((Node)cardNode2).Reparent(val2, true);
						...
						val2.AddChildSafely((Node?)(object)child2);
					}));
					break;
				}
			}
		...
		if (cardsWithoutNodesChangingPiles.Count != 0)
		{
			foreach (CardModel card4 in cardsWithoutNodesChangingPiles)
			{
				...
				Node vfxContainer = (Node)((card4.Pile.Type != PileType.Deck) ? ((object)NCombatRoom.Instance.CombatVfxContainer) : ((object)NRun.Instance.GlobalUi.TopBar.TrailContainer));
				if (val != null)
				{
					val.TweenCallback(Callable.From((Action)delegate
					{
	...
	{
		await AddCursesToDeck(new _003C_003Ez__ReadOnlySingleElementList<CardModel>(ModelDb.Card<T>()), owner);
	}
