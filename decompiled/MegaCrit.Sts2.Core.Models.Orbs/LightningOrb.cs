...
public class LightningOrb : OrbModel
{
	...
	private async Task<IEnumerable<Creature>> ApplyLightningDamage(decimal value, Creature? target, PlayerChoiceContext choiceContext)
	{
		...
		IReadOnlyList<Creature> targets = ((target == null) ? new _003C_003Ez__ReadOnlySingleElementList<Creature>(base.Owner.RunState.Rng.CombatTargets.NextItem(list)) : new _003C_003Ez__ReadOnlySingleElementList<Creature>(target));
