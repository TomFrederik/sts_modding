...
public class CrystalSphereMinigame
{
	...
	private List<Vector2I> GetAdjacentCells(int x, int y)
	{
		...
		return GetHorizontalCells(x, y).Concat(GetVerticalCells(x, y)).Concat(GetDiagonalCells(x, y)).Concat(new _003C_003Ez__ReadOnlySingleElementList<Vector2I>(new Vector2I(x, y)))
			.ToList();
	}
